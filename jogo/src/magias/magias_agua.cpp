...
// --- JatoDeAgua ---
JatoDeAgua::JatoDeAgua() : Magia("Jato de Agua", 10, 20) {}
...
void JatoDeAgua::lancar(Entidade* lancador, Entidade* alvo) {
    ...
    cout << lancador->getNome() << " dispara Jato de Agua! Dano: "
              << danoBase << " | Cura: " << cura << "\n";
}

string JatoDeAgua::getDescricao() const {
...
void OndaGelada::lancar(Entidade* lancador, Entidade* alvo) {
    ...
    cout << lancador->getNome() << " lança Onda Gelada! "
              << alvo->getNome() << " esta congelado por 1 turno!\n";
}

string OndaGelada::getDescricao() const {
    return "Dano + congela o inimigo por 1 turno.";
}

// --- CuraAgua ---
CuraAgua::CuraAgua() : Magia("Cura das Aguas", 30, 0) {}

void CuraAgua::lancar(Entidade* lancador, Entidade* alvo) {
    int cura = 50;
    lancador->setVida(lancador->getVida() + cura);
    cout << lancador->getNome() << " usa Cura das Aguas! +"
              << cura << " HP\n";
}

string CuraAgua::getDescricao() const {
    return "Recupera 50 HP instantaneamente.";
}

// --- MareAlta ---
MareAlta::MareAlta() : Magia("Mare Alta", 45, 55) {}

void MareAlta::lancar(Entidade* lancador, Entidade* alvo) {
    alvo->receberDano(danoBase);
    int cura = static_cast<int>(danoBase * 0.5f);
    lancador->setVida(lancador->getVida() + cura);
    cout << lancador->getNome() << " invoca a Mare Alta! Dano: "
              << danoBase << " | Cura: " << cura << "\n";
}

string MareAlta::getDescricao() const {
...
string Tsunami::getDescricao() const {
    return "Poder maximo da agua. Dano devastador.";
}